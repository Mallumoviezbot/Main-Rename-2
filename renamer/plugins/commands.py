...
@RenamerNs.on_message(filters.private & filters.command("ban") & filters.user(Config.ADMINS))
async def ban(c, m):
...
@RenamerNs.on_message(filters.private & filters.command("unban") & filters.user(Config.ADMINS))
async def unban(c, m):
    if len(m.command) == 1:
        await m.reply_text(
...
@RenamerNs.on_message(filters.private & filters.command("banned") & filters.user(Config.ADMINS))
